...
function init() {
	...
	new RGBELoader().setPath("").load("neutral.hdr", function (texture) {
	...
	const ClickGeometry = new PlaneGeometry(0.23, 0.665, 1, 1);
...
function playOnClick() {
	...
	const textVar = "HE'S MOVING WAIT";
	document.body.style.setProperty("--text", `"${textVar}"`);
	setTimeout(() => {
		document.querySelector(".moveprogress .bar").style.transitionDuration =
			"0.1s";
		document.querySelector(".moveprogress").classList.remove("complete");

		if (matchMedia("(pointer:fine)").matches) {
			const textVar = "CLICK AGAIN";
			document.body.style.setProperty("--text", `"${textVar}"`);
		} else {
			const textVar = "TOUCH AGAIN";
			document.body.style.setProperty("--text", `"${textVar}"`);
		}
	}, time * 1000);
}
